"""Generate the price-changes dashboard HTML from S3 CSVs and upload it back to S3.

Configuration
─────────────
BUCKET          – S3 bucket (default: bdm060897-prod)
RESULTS_PREFIX  – prefix where Athena writes query CSVs
DASHBOARD_KEY   – S3 key to write the finished HTML to

Usage (local, with AWS credentials configured):
    python terraform/lambda/dashboard_aws.py

Inside Lambda: handler is dashboard_aws.lambda_handler.
"""
...
# ── Configuration ─────────────────────────────────────────────────────────────
BUCKET         = os.environ.get("BUCKET", "bdm060897-prod")
RESULTS_PREFIX = os.environ.get("RESULTS_PREFIX", "scraper/athena-results")
DASHBOARD_KEY  = os.environ.get("DASHBOARD_KEY",  "scraper/dashboard/dashboard.html")

# ── S3 helpers ────────────────────────────────────────────────────────────────

def _read_csv_from_prefix(s3_client, prefix: str) -> pl.DataFrame:
    """Return the first CSV object found under *prefix* as a Polars DataFrame."""
    resp = s3_client.list_objects_v2(Bucket=BUCKET, Prefix=prefix)
    # Athena writes the CSV alongside a metadata file – only pick .csv keys
    keys = [o["Key"] for o in resp.get("Contents", []) if o["Key"].endswith(".csv")]
    if not keys:
        raise FileNotFoundError(f"No CSV found at s3://{BUCKET}/{prefix}")
...
def tab_section_html(df: pl.DataFrame, history_df: pl.DataFrame, is_increase: bool,
    ...
    return "".join(
        store_section_html(st, df.filter(pl.col("store") == st), history_df,
                           is_increase, col_before, col_after)
        for st in sorted(df["store"].unique().to_list())
    )


def build_html(
    result_list_increase:  pl.DataFrame,
    result_list_decrease:  pl.DataFrame,
    result_price_decrease: pl.DataFrame,
    result_best_deals:     pl.DataFrame,
    history_list_increase:  pl.DataFrame,
    history_list_decrease:  pl.DataFrame,
    history_price_decrease: pl.DataFrame,
    history_best_deals:     pl.DataFrame,
) -> str:
    # Reset chart counter so IDs are deterministic on each run
    _chart_counter[0] = 0
...
def build_and_upload():
    s3 = boto3.client("s3")
    rp = RESULTS_PREFIX.rstrip("/")

    print("Reading CSVs from S3...")
    result_list_increase  = _read_csv_from_prefix(s3, f"{rp}/list_price_increases/").rename({"pct_increase": "pct_change"})
    result_list_decrease  = _read_csv_from_prefix(s3, f"{rp}/list_price_decreases/").rename({"pct_decrease": "pct_change"})
    result_price_decrease = _read_csv_from_prefix(s3, f"{rp}/discounts/").rename({"pct_decrease": "pct_change"})
    result_best_deals     = _read_csv_from_prefix(s3, f"{rp}/avg_deals_30d/").rename({"pct_discount": "pct_change"})

    history_list_increase  = _read_csv_from_prefix(s3, f"{rp}/list_price_increases_history/")
    history_list_decrease  = _read_csv_from_prefix(s3, f"{rp}/list_price_decreases_history/")
    history_price_decrease = _read_csv_from_prefix(s3, f"{rp}/discounts_history/")
    history_best_deals     = _read_csv_from_prefix(s3, f"{rp}/avg_deals_30d_history/")

    print("Building HTML...")
    html = build_html(
        result_list_increase,  result_list_decrease,
        result_price_decrease, result_best_deals,
        history_list_increase,  history_list_decrease,
        history_price_decrease, history_best_deals,
    )

    print(f"Uploading to s3://{BUCKET}/{DASHBOARD_KEY} ...")
    s3.put_object(
        Bucket=BUCKET,
        Key=DASHBOARD_KEY,
    ...
    url = f"https://{BUCKET}.s3.amazonaws.com/{DASHBOARD_KEY}"
    print(f"Done: {url}")
    return url


def lambda_handler(event, context):
    return {"url": build_and_upload()}
